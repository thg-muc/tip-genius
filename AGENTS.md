...
- **Dead provider detection**: `_get_dead_providers` flags any provider with zero valid predictions and the workflow exits 1, after the export so successful predictions are still stored. The report wording follows `export_succeeded`, since the export can fail too. Partial failures and weeks without fixtures do not trigger it. A total odds API failure is reported separately by `_odds_api_unavailable`.
