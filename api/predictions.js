...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.KV_REST_API_URL}/get/${req.query.key || process.env.KV_DEFAULT_KEY}`,
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        error: response.status === 404 ? 'No predictions found' : 'Failed to fetch predictions',
    ...
    return res.status(500).json({
      ...
      details: process.env.NODE_ENV === 'development' ? error.message : 'Unexpected error'
    });
  }
}